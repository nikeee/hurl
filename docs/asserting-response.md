...
| __`matchesJsonSchema`__ | Query returns a JSON document valid against the JSON Schema given as predicate value<br>Schema is an inline string, a multiline string or a file | `body matchesJsonSchema file,user-schema.json;`<br><br>`` jsonpath "$.user" matchesJsonSchema ```{ "type": "object" }``` `` |
